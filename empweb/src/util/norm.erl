...
norm(Data, [#norm_rule{key=Rkey, nkey=Nkey, types=Types, default=Default}=Rule|Rest], Norm) ->
    Key = case proplists:is_defined(Rkey, Data) of
        true -> Rkey;
        _ -> convert:to_binary(Rkey)
    end,
...
to_rule_type(Value, Type_rules) ->
    to_rule_type(?CONVERTER, Value, Type_rules).
