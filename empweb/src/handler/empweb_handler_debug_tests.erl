%% 
%% @file empweb_handler_debug_tests.erl
%%  В модуле описаны различного рода тесты всей сситемы
%%  
%%  

-module(empweb_handler_debug_tests).
...
init(_, Req, _, Path_info) ->
    ...
    Is_auth         = empweb_biz_pers:is_auth(Aobj),
    User_id         = empweb_biz_pers:get_pers_id(Aobj),
    User_perm_names = empweb_biz_pers:get_perm_names(Aobj),
    Hap = case Path_info of
        %% для тестирование empweb_jsonapi с помощью утилиты ab
        [<<"empweb_jsonapi_pers">>, <<"get_all_perss">>] ->
                #empweb_hap{
...
%% Былы создана для тестирование empweb_jsonapi с помощью утилиты ab
%%
handle_hap(Req, #empweb_hap{handler=empweb_jsonapi_pers,action=get_all_perss}=Hap)->
    Empweb_resp = case empweb_http:call(Req, Hap) of
        {ok, R} -> R;
        {error, unknown_function} ->
            empweb_jsonapi:not_extended(unknown_function);
        {error, Error} ->
            empweb_jsonapi:internal_server_error(
                {[{unknown_error, empweb_jsonapi:format(Error)}]}
            )
    end,
    {ok, Empweb_resp, Hap}.
