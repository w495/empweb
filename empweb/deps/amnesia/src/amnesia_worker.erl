...
set_config(Options) ->
    lists:map(
        fun({Name, Value}) ->
            term_cache_ets:put(amnesia_config, Name, Value)
        end,
        Options
    ),
    ok.
...
cluster(ClusterNodes) ->
  ...
  try
    amnesia_flog:info([{?MODULE,cluster},{nodes,ClusterNodes}]),
...
global_tables()   ->
    case term_cache_ets:get(amnesia_config, global_tables) of
        not_found ->
            ?GLOBAL_TABLES;
        {ok, Result} ->
            Result
...
local_tables()    -> 
    case term_cache_ets:get(amnesia_config, local_tables) of
        not_found ->
            ?LOCAL_TABLES;
        {ok, Result} ->
            Result
            ...
            case mnesia:system_info(use_dir) of
                true ->
                    case check_schema_integrity() of
                        ok -> ok;
                        {error, Reason} ->
                            %% we cannot use amnesia_flog here since
...
move_db() ->
    ...
    case file:rename(MnesiaDir, BackupDir) of
        ok ->
            %% we cannot use amnesia_flog here since it may not have
            %% been started yet
