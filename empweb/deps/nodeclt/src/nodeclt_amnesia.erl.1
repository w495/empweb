...
cluster(ClusterNodes) ->
  ...
  try
    nodeclt_flog:info([{?MODULE,cluster},{nodes,ClusterNodes}]),
            ...
            case mnesia:system_info(use_dir) of
                true ->
                    case check_schema_integrity() of
                        ok -> ok;
                        {error, Reason} ->
                            %% we cannot use nodeclt_flog here since
...
move_db() ->
    ...
    case file:rename(MnesiaDir, BackupDir) of
        ok ->
            %% we cannot use nodeclt_flog here since it may not have
            %% been started yet
