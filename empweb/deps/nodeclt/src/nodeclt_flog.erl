...
% Old API, redirect to nodeclt_clog
...
% Old API functions
%----------------------------------------------------------------------
info(Arg) -> nodeclt_clog:info(Arg).

error(Arg) ->
    nodeclt_clog:error(Arg).

debug(Arg) -> nodeclt_clog:debug(Arg).
debug(Arg1,Arg2) -> nodeclt_clog:debug(Arg1,Arg2).
debug(Arg1,Arg2,Arg3) -> nodeclt_clog:debug(Arg1,Arg2,Arg3).
...
init([App]) ->
    ...
    LF = case application:get_env(App, flog_path) of
       {ok,F} -> F;
       _      -> "./priv/logs/nodeclt_flog.log"
         end,
%    io:format("~p~n", [{nodeclt_flog,lf,LF}]),
...
%    io:format("nodeclt_flog: file:write()=~p~n", [Rc]),
