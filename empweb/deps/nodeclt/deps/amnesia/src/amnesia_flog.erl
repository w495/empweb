...
% Old API, redirect to amnesia_clog
...
% Old API functions
%----------------------------------------------------------------------
info(Arg) -> amnesia_clog:info(Arg).

error(Arg) ->
    amnesia_clog:error(Arg).

debug(Arg) -> amnesia_clog:debug(Arg).
debug(Arg1,Arg2) -> amnesia_clog:debug(Arg1,Arg2).
debug(Arg1,Arg2,Arg3) -> amnesia_clog:debug(Arg1,Arg2,Arg3).
...
init([App]) ->
    Daemon = chk_daemon(),
    LF = case application:get_env(App, amnesia_flog_path) of
       {ok,F} -> F;
        _      ->
            Res = "./priv/logs/" ++ erlang:atom_to_list(node()) ++ "/amnesia_flog.log",
            filelib:ensure_dir(Res),
            Res 
         end,
%    io:format("~p~n", [{amnesia_flog,lf,LF}]),
...
%          error_logger:tty(true),
      error_logger:error_report([{amnesia_flog_file, LF}, Error]),
...
%    io:format("amnesia_flog: file:write()=~p~n", [Rc]),
