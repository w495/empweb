...
**/
create or replace function cptrans_util_fields_on_insert() returns "trigger" as $$
begin
...
drop trigger if exists t1cptrans_util_fields_on_insert on cptrans ;
create trigger t1cptrans_util_fields_on_insert before insert
on cptrans for each row execute procedure cptrans_util_fields_on_insert();

create or replace function cptrans_util_fields_on_update() returns "trigger" as $$
begin
    /**
        Владелец заявки
    **/
...
drop trigger if exists t1cptrans_util_fields_on_update on cptrans ;
create trigger t1cptrans_util_fields_on_update before update
on cptrans for each row execute procedure cptrans_util_fields_on_update();
