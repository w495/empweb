...
get(Con, What) ->
    ...
    %         "   join flag on flag.doc_id = doc.id "
    %         "   join file on file.id = flag.file_id "
    ...
    Fields =
        proplists:get_value(
            fields,
            What,
            lists:append([empdb_dao_flag:table({fields, select}), empdb_dao_doc:table({fields, select})])
        ),

    case empdb_dao:get([
        {empdb_dao_doc, id},
        {empdb_dao_flag, {doc_id, file_id}},
        {empdb_dao_file, id},
