...
get(Con, What) ->
    ...
    %         "   join back on back.doc_id = doc.id "
    %         "   join file on file.id = back.file_id "
    ...
    Fields =
        proplists:get_value(
            fields,
            What,
            lists:append([empdb_dao_back:table({fields, select}), empdb_dao_doc:table({fields, select})])
        ),

    case empdb_dao:get([
        {empdb_dao_doc, id},
        {empdb_dao_back, {doc_id, file_id}},
        {empdb_dao_file, id},
