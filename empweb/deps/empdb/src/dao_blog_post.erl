...
create(Con, Proplist)->
    case dao_doc:create(Con,doc_pl(Proplist)) of
        {ok, Doc_id} ->
            create_blog_post(Con,[{doc_id, Doc_id} | blog_post_pl(Proplist)]);
        {error, Error} ->
            Error
    end.

create_blog_post(Con, Proplist)->
...
update_(Con, Proplist)->
    case dao_doc:update(Con,doc_pl(Proplist)) of
        {ok, Doc_id} ->
            update_blog_post(Con, Doc_id, [{doc_id, Doc_id}| blog_post_pl(Proplist)]);
        {error, Error} ->
            Error
    end.

update_blog_post(Con, Doc_id, Proplist)->
    Fields = fields(Proplist),
    dao:pgret(
        dao:equery(Con,[
