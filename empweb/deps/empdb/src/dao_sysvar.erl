...
%%%
get(Uniq, Default) ->
    dao:with_connection_fk(fun(Con)-> get(Con, Uniq, Default) end).
...
%%%
get_value(Uniq) ->
    dao:with_connection_fk(fun(Con)-> get_value(Con, Uniq)  end).
...
%%%
get_sysvars(Arg) ->
    dao:with_connection_fk(fun(Con)-> get_sysvars(Con, Arg) end).
...
get_sysvars(Con, {perm, Perm}) ->
    Query =
        ...
        " ; ",
    dao:eqret(Con, Query, [convert:to_list(Perm)]);
...
%%%
get_sysvar(Arg) ->
    dao:with_connection_fk(fun(Con)-> get_sysvar(Con, Arg) end).

get_sysvar(Con, Name) when erlang:is_atom(Name) ->
    get_sysvar(Con, convert:to_list(Name));
...
%%%
update_sysvar(Arg) ->
    dao:with_connection_fk(fun(Con)-> update_sysvar(Con, Arg) end).
...
update_sysvar_unsafe(Arg) ->
    dao:with_connection_fk(fun(Con)-> update_sysvar_unsafe(Con, Arg) end).

update_sysvar_unsafe(Con, {Name, Value})
        when erlang:is_atom(Name) ->
    update_sysvar_unsafe(Con, {convert:to_list(Name), Value});
...
update_sysvar_unsafe(Con, {Id, Value})
    ...
    Query =
        ...
        " where "
            " id = $1;",
    dao:eqret(Con, Query, [Id, convert:to_list(Value)]);
...
update_sysvar_unsafe(Con, {Name, Value})
    ...
    Query =
        ...
        " where "
            " name = $1;",
    dao:eqret(Con, Query, [Name, convert:to_list(Value)]).
